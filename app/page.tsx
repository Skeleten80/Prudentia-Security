...
export default function Page() {
  return <main className='p-8 space-y-8 max-w-7xl mx-auto'>
    ...
    <GlassCard className='p-6'><h2 className='text-xl mb-3'>Pricing</h2><div className='grid md:grid-cols-5 gap-3'>{plans.map((p)=><div className='bg-white/5 rounded-xl p-3' key={p.name}><div>{p.name}</div><div className='text-2xl'>{p.price}</div><p className='text-xs text-white/70 mt-1'>{p.description}</p></div>)}</div></GlassCard>
  </main>;
}
